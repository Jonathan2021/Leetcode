 ...
 * =====================================================================================
 *
 *       Filename:  FindFirstLastPositionElementSortedArray.cpp
 *
 *    Description:  https://leetcode.com/problems/find-first-and-last-position-of-element-in-sorted-array/submissions
 *
 *        Version:  1.0
 *        Created:  09/17/22 12:06:24
 *       Revision:  none
 *       Compiler:  gcc
 *
...
vector<int> binary_search(vector<int>& nums, int target, int left, int right)
{
    if (left <= right)
    {
        ...
        {
            int more_left = binary_search(nums, target, left, consider - 1)[0];
            int more_right = binary_search(nums, target, consider +1, right)[1];
            left = right = consider;
            if (more_left != -1)
                left = more_left;
            if (more_right != -1)
        ...
        if (nums[consider] < target)
            return binary_search(nums, target, consider + 1, right);
        return binary_search(nums, target, left, consider -1);
    }
    return {-1, -1};
}
